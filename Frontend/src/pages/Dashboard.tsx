...
import { Activity, LogOut, Settings, Server, AlertCircle, X } from "lucide-react";
      ...
      <aside style={{ width: '280px', borderRight: '1px solid var(--border-strong)', background: 'rgba(10, 10, 10, 0.6)', backdropFilter: 'blur(20px)', padding: '1.5rem', display: 'flex', flexDirection: 'column', position: 'relative', zIndex: 10 }}>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: 1 }}>
            {[
                ...
                const isActive = item.name === "Endpoints"; // Dashboard handles Endpoints/Connectivity Testing for now
                return (
                            ...
                            <div key={ep.id} style={{ background: 'rgba(255,255,255,0.02)', padding: '1rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                                        <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: ep.status === 'UP' ? '#10b981' : ep.status === 'DOWN' ? '#ef4444' : '#fbbf24' }}></div>
